...
def connect_db(db_path, target_schema):
    ...
    for candidate in candidates:
        ...
        if os.path.exists(candidate_abs) and os.path.isfile(candidate_abs):
            ...
            return SQLDatabase.from_uri("sqlite:///" + candidate_abs)
...
def prioritize_tables(documents, target_schema, sql_database):
    ...
    table_sorting = set(
        ...
        if doc.metadata["schema"] == target_schema
    )
    ...
    priority_tables = sorted(
        table_names,
        key=lambda x: (table_indices.get(x, float("inf")), table_names.index(x)),
    )
...
def _call_hf_raw(lang_model, prompt, params=None):
    """
    Use huggingface_hub InferenceApi client with raw_response=True and
    return a best-effort string result.
    """
    client = getattr(lang_model, "client", None)
    if client is None:
        raise AttributeError("Provided lang_model has no .client attribute")

    params = params or {}
    resp = client(inputs=prompt, params=params, raw_response=True)

    # resp is a requests.Response-like object; try to decode content/text
    try:
        content = (
            resp.content.decode("utf-8") if hasattr(resp, "content") else str(resp)
        )
    except Exception:
        content = resp.text if hasattr(resp, "text") else str(resp)

    # try JSON parse and extract common fields
    try:
...
def llm_create_sql(sql_dialect, table_info, question, lang_model):
    ...
    # If the passed lang_model exposes a huggingface client, call raw and parse
    if hasattr(lang_model, "client"):
        try:
            sql_query = _call_hf_raw(lang_model, prompt_text)
            return sql_query
        except Exception:
            # fall back to LLMChain behavior if anything unexpected happens
            pass

    create_prompt = PromptTemplate(input_variables=[], template=prompt_text)
...
def llm_check_sql(sql_query, sql_dialect, lang_model):
    ...
    if hasattr(lang_model, "client"):
        try:
            checked_sql = _call_hf_raw(lang_model, prompt_text)
            return checked_sql
        except Exception:
            pass

    validate_prompt = PromptTemplate(input_variables=[], template=prompt_text)
...
def llm_debug_error(sql_query, error_message, lang_model):
    ...
    if hasattr(lang_model, "client"):
        try:
            return _call_hf_raw(lang_model, debug_text)
        except Exception:
            pass

    debug_error_prompt = PromptTemplate(input_variables=[], template=debug_text)
    debug_error_chain = LLMChain(
        llm=lang_model, prompt=debug_error_prompt, verbose=False
    )
...
def llm_debug_empty(sql_query, question, lang_model):
    ...
    debug_empty_prompt = PromptTemplate(input_variables=[], template=debug_text)
    debug_empty_chain = LLMChain(
        llm=lang_model, prompt=debug_empty_prompt, verbose=False
    )
...
def llm_analyze(query_result, question, lang_model):
    ...
    if hasattr(lang_model, "client"):
        try:
            return _call_hf_raw(lang_model, prompt_text)
        except Exception:
            pass

    analyze_prompt = PromptTemplate(input_variables=[], template=prompt_text)
