...
PERSIST_DIR = r"D:\text-sql\src\data\processed\chromadb"
DB_FILEPATH = r"D:\text-sql\src\data\raw\spider\database"
...
def sql_copilot(language_model=llm, max_attempts=3):
    ...
    db_documents = similar_doc_search(
        question=user_question, vector_database=vectordb, top_k=3
    )
    ...
    result = "FAIL"  # set a default result
    ...
    for schema in list(top_schemas):  # its in a set, so have to convert to a list
        ...
        tables_sorted = prioritize_tables(
            documents=db_documents, target_schema=schema, sql_database=db
        )
        ...
        validated_sql = llm_check_sql(
            sql_query=sql_statement, sql_dialect=sql_dialect, lang_model=language_model
        )
        print("...SQL validated.")
