...
def get_table_schema(state: PassedState):
    ...
    data = curr.fetchall()
    for table, column, dtype in data:
...
def get_relevant_table(state: PassedState):
    prompt = f"""you are part of text to sql agent. you wil be given a question {state['question']}. 
    here is the list of tables in the database: {state['schema']}
    based on the question and the tables, identify the most relevant table for the query. and give a list of the table names only and nothing else. we only have 2 necessart tables rn: 'students'.
    """
    data = {
        "model": "llama3.2",
        "prompt": prompt,
    ...
    tables = response.json().get("response").strip()
...
def get_the_query(state: PassedState):
    ...
    prompt = (
        ...
        f"tables involved: {state['tables']}. "
...
def give_final_answer(state: PassedState):
    if state['error']:
        ...
        prompt = f"you will be given output of text to sql query executed from table students. here is the schema {state['schema']}. and here is the result of the sql query {state['result']}. and here is the query that was executed {state['query']}. rephrase the answer in natural language for the user to understand. give a very short answer."
        data = {
            "model": "llama3.2",
            "prompt": prompt,
            "stream": False
